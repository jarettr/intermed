...
///
/// `c` and `common` are the cross-loader convention tag namespaces; `forge` /
/// `neoforge` / `fabric` / `quilt` are loader-provided. Treating these as
/// installable mods would manufacture false "missing namespace" findings — see
/// the namespace-ownership notes in the roadmap.
...
mod tests {
    ...
    fn path_namespace_extraction() {
        ...
        assert_eq!(
            path_namespace("assets/sodium/lang/en_us.json").as_deref(),
            Some("sodium")
        );
    ...
    fn platform_namespaces_recognised() {
        ...
        assert!(is_platform_namespace("neoforge"));
