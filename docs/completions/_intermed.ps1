...
Register-ArgumentCompleter -Native -CommandName 'intermed' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'intermed' {
            [CompletionResult]::new('--config', '--config', [CompletionResultType]::ParameterName, 'Config file (`intermed-config-v1` TOML). Overrides discovery; see docs/CONFIG.md')
            ...
            [CompletionResult]::new('deps', 'deps', [CompletionResultType]::ParameterValue, 'Layer-C dependency graph and PubGrub resolution')
        ...
        'intermed;doctor' {
            ...
            [CompletionResult]::new('--logic', '--logic', [CompletionResultType]::ParameterName, 'Rule backend selection. Imperative remains the stable fallback')
            ...
            [CompletionResult]::new('--minecraft-jar', '--minecraft-jar', [CompletionResultType]::ParameterName, 'Minecraft client/server jar to index for mixin apply-failure verification. Without it, apply-failure checks cover only mod-targeting mixins')
