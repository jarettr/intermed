...
Register-ArgumentCompleter -Native -CommandName 'intermed' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        ...
        'intermed;doctor' {
            ...
            [CompletionResult]::new('--pack-manifest', '--pack-manifest', [CompletionResultType]::ParameterName, 'Authoritative original pack manifest or archive. Use this when analyzing a materialized instance whose `modrinth.index.json`/`manifest.json` was not retained by the launcher or extraction pipeline')
            ...
            [CompletionResult]::new('--telemetry-out', '--telemetry-out', [CompletionResultType]::ParameterName, 'Explicitly export a privacy-filtered telemetry event to FILE. Nothing is collected or written unless this option or `--telemetry-endpoint` is supplied')
            [CompletionResult]::new('--telemetry-endpoint', '--telemetry-endpoint', [CompletionResultType]::ParameterName, 'Explicitly send a privacy-filtered telemetry event to an HTTPS endpoint. No built-in service is contacted; the destination must be supplied here')
            ...
            [CompletionResult]::new('--mixin-level', '--mixin-level', [CompletionResultType]::ParameterName, 'Mixin analysis preset: `basic` (overlaps/risk only), `standard` (+ recommendations), `full` (+ per-handler intelligence findings)')
            ...
            [CompletionResult]::new('--config', '--config', [CompletionResultType]::ParameterName, 'Config file (`intermed-config-v1` TOML). Overrides discovery; see docs/reference/configuration.md')
            [CompletionResult]::new('--mixin-risk', '--mixin-risk', [CompletionResultType]::ParameterName, 'Compatibility shorthand for `--mixin-level standard`')
            [CompletionResult]::new('--no-color', '--no-color', [CompletionResultType]::ParameterName, 'Disable ANSI colour even on a TTY')
            [CompletionResult]::new('--telemetry-include-log-excerpts', '--telemetry-include-log-excerpts', [CompletionResultType]::ParameterName, 'Include up to 20 redacted, truncated log-signal excerpts in explicitly requested telemetry. Requires `--telemetry-out` or `--telemetry-endpoint`')
